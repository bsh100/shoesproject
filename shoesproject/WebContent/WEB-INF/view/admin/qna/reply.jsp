...
<%
	QnaDto qnaObj = (QnaDto)request.getAttribute("qna");
...
<body>
	...
	<div class="registBox">
		<form>
			<label>제목</label> 
			<input type="hidden" name="team" value="<%=qnaObj.getTeam()%>">
         	<input type="hidden" name="rank" value="<%=qnaObj.getRanking()%>">
         	<input type="hidden" name="depth" value="<%=qnaObj.getDepth()%>">
			<input type="text" id="qnaTitle" name="qnaTitle" placeholder="제목을 입력해 주세요"> 
			<label>작성자</label> 
			<input type="text" id="qnaWriter" name="qnaWriter" value="<%=userId%>" disabled="disabled">
			<label>내용</label>
